...
if models_dir not in sys.path:
    ...
    print(f"添加模型路径到系统路径: {models_dir}")
...
def detect(opt):
    ...
    t0 = time.time()
    # 提前加载人脸数据库
    from face_db import init_db,get_all_faces,add_face_from_encoding
    import face_recognition
    import numpy as np
    
    # 初始化数据库
    init_db(db_path=r'faces.db')
    # 获取数据
    known_ids, known_encodings = [], []
    for face_id, encoding, *_ in get_all_faces():  # 直接解包三元组
       known_ids.append(face_id)
       known_encodings.append(encoding)
    print(f"数据库已加载 {len(known_encodings)} 条人脸数据")   
    if len(known_encodings) > 0:
        print(f"示例数据: ID={known_ids[0]},  Encoding长度={len(known_encodings[0])}")
    else:
        print("警告: 数据库为空!") 
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        if frame_count % 30 == 0:
            ...
            for face_id, encoding in get_all_faces():
                known_ids.append(face_id)
                known_encodings.append(encoding)
            ...
            if len(det):
                ...
                for det_index, (*xyxy, conf, cls) in enumerate(reversed(det[:,:6])):
                    ...
                    if save_img or opt.save_crop or view_img:  # Add bbox to image
                        ...
                        base_label = None
                        kpts = None 
                        if kpt_label and det.shape[1] > 6:  # 检查是否有关键点数据
                            kpts = det[det_index, 6:]  # 从检测结果中提取关键点
                        if not opt.hide_labels:
                            base_label = f'{names[c]} {conf:.2f}' if not opt.hide_conf else names[c]
                        ...
                        if c == 0:  # 人脸类别
                            # 转换坐标并裁剪人脸区域
                            face_coords = scale_coords(img.shape[2:], torch.tensor([xyxy]), im0.shape)[0].cpu().numpy()
                            h, w = im0.shape[:2]
                            x1, y1, x2, y2 = map(int, [max(0,face_coords[0]), max(0,face_coords[1]), 
                                                     min(w,face_coords[2]), min(h,face_coords[3])])
                                ...
                                if face_encodings:
                                    ...
                                    if known_encodings:
                                        ...
                                        if face_distances[best_match_index] < 0.6:
                                            id_label = f'ID: {known_ids[best_match_index]}'
                                        else:
                                             # 自动保存未知人脸
                                            new_id = add_face_from_encoding(current_encoding)  # 新增函数
                                            id_label = f'ID: {new_id} (新增)'
                                            # 立即更新数据库缓存
                                            known_ids.append(new_id)
                                            known_encodings.append(current_encoding)
                                    else:
                                        new_id = add_face_from_encoding(current_encoding)
                                        id_label = f'ID: {new_id} (新增)'
                                        known_ids.append(new_id)
                                        known_encodings.append(current_encoding)
                            else:
                                id_label += 'ID:未提取特征'
                        plot_one_box(xyxy, im0, label=base_label, color=colors(c, True), line_thickness=opt.line_thickness, 
                                       kpt_label=kpt_label, kpts=kpts, steps=3, orig_shape=im0.shape[:2])
                        if id_label:
                            x1_box, y1_box = int(xyxy[0]), int(xyxy[1])
                            tf = max(opt.line_thickness - 1, 1)
                            # 在检测框下方绘制ID信息
                            cv2.putText(
                                im0, 
                                id_label, 
                                (x1_box, y1_box + 20),  # 调整位置避免重叠
